<?php

declare(strict_types=1);

namespace MarcoConsiglio\BCMathExtended\Tests\Unit;

use MarcoConsiglio\BCMathExtended\Number;
use BcMath\Number as BcMathNumber;
use InvalidArgumentException;
use PHPUnit\Framework\Attributes\DataProvider;
use PHPUnit\Framework\TestCase;

class NumberTest extends TestCase
{
    protected function setUp(): void
    {
        // Number::setScale(2);
    }

    public static function scientificNotationProvider(): array
    {
        return [
            ['0', '-0'],
            ['0', ''],
            ['666', '666'],
            ['-666', '-666'],
            [
                '99999999999999999999999999999999999.000000000000000000000',
                '99999999999999999999999999999999999.000000000000000000000',
            ],
            [
                '99999999999999999999999999999999999.999999999999999999999',
                '99999999999999999999999999999999999.999999999999999999999',
            ],
            ['1000000000000000000000000000000', '1.0E+30'],
            ['-1540000000000000', '-1.54E+15'],
            ['1540000000000000', '1.54E+15'],
            ['602200000000000000000000', '6.022E+23'],
            ['602200000000000000000000', '6.022e+23'],
            ['-602200000000000000000000', '-6.022e+23'],
            ['-602200000000000000000000', '-6.022E+23'],
            ['1999.99', '19.9999E+2'],
            ['0.00000000001', '1.0E-11'],
            ['0.0000051', '5.1E-6'],
            ['-0.00051', '-5.1E-4'],
            ['0.02', '2E-2'],
            ['0.0021', '2.1E-3'],
            ['0.00000003', '3E-8'],
            ['0.000000657', '6.57E-7'],
            ['5', '5e+0'],
            ['-5', '-5e+0'],
            ['5.254', '5.254e+0'],
            ['8853.6719', '8.8536719e+3'],
            ['0.00000000001', '0.00000000001'],
            ['-0.00116000', '-0.00116000'],
            ['-26.2912940386', '-2.62912940386e+1'],
            ['2.6', '2.6e+0'],
            ['1734825599220.52', '1.73482559922052e+12'],
            ['-57170562.129942072027205098329198887303', '-5.7170562129942072027205098329198887303e+7'],
            ['0.000021', '2.1e-5'],
            ['0.7811084054', '7.811084054e-1'],
            ['0', '0e+0'],
            ['-1.1', '-1.1e+0'],
            ['-4.182', '-4.182e+0'],
            ['23.07', '2.307e+1'],
            ['-2349', '-2.349e+3'],
            ['-230807.1307795', '-2.308071307795e+5'],
            ['-887126.1', '-8.871261e+5'],
            [
                '-0.40559318155029357183161762311247760893712676112986144952',
                '-4.0559318155029357183161762311247760893712676112986144952e-1',
            ],
            [
                '-749168762.7771507445838618797279002344648652959333491',
                '-7.491687627771507445838618797279002344648652959333491e+8',
            ],
            ['1.3333', '0.13333e+01'],
            ['100', '1e+2'],
            // some rubbish..
            ['23', '23.1.23.0e+0..3123131'], //hmm
            ['345600000000', '3.456e11'],
            ['345600000000', '3.456e11'],
            ['-345600000000', '-3.456e11'],
            ['-345600000000', '-3.456e11'],

        ];
    }

    #[DataProvider('scientificNotationProvider')]
    public function testConvertScientificNotationToString(string $expected, string $number): void
    {
        self::assertSame($expected, (string)Number::convertToNumber($number));
    }

    public static function ceilProvider(): array
    {
        return [
            ['0', '-0'],
            ['-1', '-1'],
            ['-1', '-1.5'],
            ['-1', '-1.8'],
            ['-2', '-2.7'],
            ['0', '0'],
            ['1', '0.5'],
            ['1', '1'],
            ['2', '1.5'],
            ['2', '1.8'],
            ['3', '2.7'],
            ['0', '-0'],
            ['0', ''],
            ['0', 'null'],
            ['20000', '2/0000'],
            ['-60000', '-6/0000'],
            ['1000000000000000000000000000000', '+1/000000000000000000000000000000'],
            ['99999999999999999999999999999999999', '99999999999999999999999999999999999.000000000000000000000'],
            ['100000000000000000000000000000000000', '99999999999999999999999999999999999.999999999999999999999'],
            ['0', '0-'],
            ['100000000000000000000000000000000000', '1.0E+35'],
            ['-100000000000000000000000000000000000', '-1.0E+35'],
            ['1', '3E-8'],
            ['1', '1.0E-11'],
        ];
    }

    #[DataProvider('ceilProvider')]
    public function testCeil(string $expected, string $number): void
    {
        self::assertSame($expected, (string)Number::ceil($number));
    }

    public static function floorProvider(): array
    {
        return [
            ['0', '-0'],
            ['-1', '-0.5'],
            ['-1', '-1'],
            ['-2', '-1.5'],
            ['-2', '-1.8'],
            ['-3', '-2.7'],
            ['0', '0'],
            ['0', '0.5'],
            ['1', '1'],
            ['1', '1.5'],
            ['1', '1.8'],
            ['2', '2.7'],
            ['0', '-0'],
            ['0', ''],
            ['0', 'null'],
            ['20000', '2/0000'],
            ['-60000', '-6/0000'],
            ['1000000000000000000000000000000', '+1/000000000000000000000000000000'],
            ['99999999999999999999999999999999999', '99999999999999999999999999999999999.000000000000000000000'],
            ['99999999999999999999999999999999999', '99999999999999999999999999999999999.999999999999999999999'],
            ['0', '0-'],
            ['100000000000000000000000000000000000', '1.0E+35'],
            ['-100000000000000000000000000000000000', '-1.0E+35'],
            ['0', '3E-8'],
            ['0', '1.0E-11'],
        ];
    }

    #[DataProvider('floorProvider')]
    public function testFloor(string $expected, string $number): void
    {
        self::assertSame($expected, (string)Number::floor($number));
    }

    public static function absProvider(): array
    {
        return [
            ['1', '-1'],
            ['1.5', '-1.5'],
            ['1', '-1'],
            ['1.5', '-1.5'],
            [
                '9999999999999999999999999999999999999999999999999999999',
                '-9999999999999999999999999999999999999999999999999999999',
            ],
            ['0', '-0'],
            ['0', ''],
            ['0', 'null'],
            ['20000', '2/0000'],
            ['60000', '-6/0000'],
            ['1000000000000000000000000000000', '+1/000000000000000000000000000000'],
            ['0', '0-'],
            ['100000000000000000000000000000000000', '1.0E+35'],
            ['100000000000000000000000000000000000', '-1.0E+35'],
            ['0.0000051', '-5.1E-6'],
        ];
    }

    #[DataProvider('absProvider')]
    public function testAbs(string $expected, string $number): void
    {
        self::assertSame($expected, (string)Number::abs($number));
    }

    public static function roundProvider(): array
    {
        return [
            ['3', '3.4'],
            ['4', '3.5'],
            ['4', '3.6'],
            ['2', '1.95583'],
            ['2', '1.95583'],
            ['1.96', '1.95583', 2],
            ['1.956', '1.95583', 3],
            ['1.9558', '1.95583', 4],
            ['1.95583', '1.95583', 5],
            ['1241757', '1241757'],
            ['1241757', '1241757', 5],
            ['-3', '-3.4'],
            ['-4', '-3.5'],
            ['-4', '-3.6'],
            ['123456.745671', '123456.7456713', 6],
            ['1', '1.11'],
            ['1.11', '1.11', 2],
            ['0.1666666666667', '0.1666666666666665', 13],
            ['0', '0.1666666666666665', 0],
            ['10', '9.999'],
            ['10', '9.999', 2],
            ['0.01', '0.005', 2],
            ['0.02', '0.015', 2],
            ['0.03', '0.025', 2],
            ['0.04', '0.035', 2],
            ['0.05', '0.045', 2],
            ['0.06', '0.055', 2],
            ['0.07', '0.065', 2],
            ['0.08', '0.075', 2],
            ['0.09', '0.085', 2],
            ['77777777777777777777777777777', '77777777777777777777777777777.1'],
            [
                '100000000000000000000000000000000000',
                '99999999999999999999999999999999999.99999999999999999999999999999999991',
            ],
            [
                '99999999999999999999999999999999999',
                '99999999999999999999999999999999999.00000000000000000000000000000000001',
            ],
            ['99999999999999999999999999999999999', '99999999999999999999999999999999999.000000000000000000000'],
            ['0', '-0'],
            ['0', ''],
            ['0', 'null'],
            ['20000', '2/0000'],
            ['-60000', '-6/0000'],
            ['1000000000000000000000000000000', '+1/000000000000000000000000000000'],
            ['0', '0-'],
            ['100000000000000000000000000000000000', '1.0E+35'],
            ['-100000000000000000000000000000000000', '-1.0E+35'],
            ['0', '3E-8'],
            ['0', '1.0E-11'],
            ['-0.0006', '-5.6E-4', 4],
            ['0.000000001', '9.9999E-10', 10],
        ];
    }

    #[DataProvider('roundProvider')]
    public function testRound(string $expected, string $number, int $precision = 0): void
    {
        self::assertSame($expected, (string)Number::round($number, $precision));
    }

    public static function roundHalfEvenProvider(): array
    {
        return [
            ['2', '1.8'],
            ['2', '1.5'],
            ['1', '1.2'],
            ['1', '0.8'],
            ['0', '0.5'],
            ['0', '0.2'],
            ['0', '-0.2'],
            ['0', '-0.5'],
            ['-1', '-0.8'],
            ['-1', '-1.2'],
            ['-2', '-1.5'],
            ['-2', '-1.8'],
            ['-2.35', '-2.35', 2],
            ['-2.4', '-2.35', 1],
            ['2.4', '2.35', 1],
            ['0.00', '0.005', 2],
            ['0.02', '0.015', 2],
            ['0.02', '0.025', 2],
            ['0.04', '0.035', 2],
            ['0.04', '0.045', 2],
            ['0.06', '0.055', 2],
            ['0.06', '0.065', 2],
            ['0.08', '0.075', 2],
            ['0.08', '0.085', 2],
            ['1.13', '1.1259', 2],
            ['100', '100', 0],
        ];
    }

    #[DataProvider('roundHalfEvenProvider')]
    public function testRoundHalfEven(string $expected, string $number, int $precision = 0): void
    {
        self::assertSame($expected, (string)Number::roundHalfEven($number, $precision));
    }

    public static function randProvider(): array
    {
        return [
            ['1', '3'],
            ['432423432423423423423423432432423423423', '999999999999999999999999999999999999999999'],
        ];
    }

    #[DataProvider('randProvider')]
    public function testRand(string $left, string $right): void
    {
        $rand = (string)Number::rand($left, $right);
        self::assertTrue($rand >= $left);
        self::assertTrue($rand <= $right);
    }

    public function test_something(): void
    {
        $a = new BcMathNumber("10");
        $b = new BcMathNumber("7");

        $this->assertTrue($a > $b);
    }

    public function testMax(): void
    {
        self::assertSame('3', (string)Number::max(1, 2, 3));
        self::assertSame('6', (string)Number::max(6, 3, 2));
        self::assertSame('999', (string)Number::max(100, 999, 5));

        self::assertSame('677', (string)Number::max([3, 5, 677]));
        self::assertSame('-3', (string)Number::max([-3, -5, -677]));

        self::assertSame(
            '999999999999999999999999999999999999999999',
            (string)Number::max(
                '432423432423423423423423432432423423423',
                '999999999999999999999999999999999999999999',
                '321312312423435657'
            )
        );
        self::assertSame('0.00000000099999', (string)Number::max(9.9999E-10, -5.6E-4));
    }

    public function testMin(): void
    {
        self::assertSame('7.20', (string)Number::min('7.30', '7.20'));
        self::assertSame('3', (string)Number::min([3, 5, 677]));
        self::assertSame('-677', (string)Number::min([-3, -5, -677]));

        self::assertSame(
            '321312312423435657',
            (string)Number::min(
                '432423432423423423423423432432423423423',
                '999999999999999999999999999999999999999999',
                '321312312423435657'
            )
        );

        self::assertSame('-0.00056', (string)Number::min(9.9999E-10, -5.6E-4));
    }

    public static function setScaleProvider(): array
    {
        return [[50, '3', '1', '2'], [0, '3', '1', '2'], [13, '3', '1', '2']];
    }

    #[DataProvider('setScaleProvider')]
    public function testSetScale(int $scale, string $expected, string $left, string $right): void
    {
        Number::setScale($scale);
        self::assertSame($expected, (string) (new Number($left))->add($right));
    }

    public static function roundUpProvider(): array
    {
        return [
            ['663', '662.79'],
            ['662.8', '662.79', 1],
            ['60', '54.1', -1],
            ['60', '55.1', -1],
            ['-23.6', '-23.62', 1],
            ['4', '3.2'],
            ['77', '76.9'],
            ['3.142', '3.14159', 3],
            ['-3.1', '-3.14159', 1],
            ['31500', '31415.92654', -2],
            ['31420', '31415.92654', -1],
            ['0.0119', '0.0119', 4],
            ['0', '-0'],
            ['0', ''],
            ['0', 'null'],
            ['0', '0-'],
            ['1', '9.9999E-10'],
        ];
    }

    #[DataProvider('roundUpProvider')]
    public function testRoundUp(string $expected, string $number, int $precision = 0): void
    {
        self::assertSame($expected, (string)Number::roundUp($number, $precision));
    }

    public static function roundDownProvider(): array
    {
        return [
            ['662', '662.79'],
            ['662.7', '662.79', 1],
            ['50', '54.1', -1],
            ['50', '55.1', -1],
            ['-23.7', '-23.62', 1],
            ['3', '3.2'],
            ['76', '76.9'],
            ['3.141', '3.14159', 3],
            ['-3.2', '-3.14159', 1],
            ['31400', '31415.92654', -2],
            ['31410', '31415.92654', -1],
            ['0.0119', '0.0119', 4],
            ['0', '-0'],
            ['0', ''],
            ['0', 'null'],
            ['0', '0-'],
            ['0', '9.9999E-10'],
            ['1.12', '1.1259', 2],
        ];
    }

    #[DataProvider('roundDownProvider')]
    public function testRoundDown(string $expected, string $number, int $precision = 0): void
    {
        self::assertSame($expected, (string)Number::roundDown($number, $precision));
    }

    public static function addProvider(): array
    {
        return [
            ['3', '1', '2'],
            ['2', '1', '1'],
            ['15', '10', '5'],
            ['2.05', '1', '1.05', 2],
            ['4', '-1', '5', 4],
            ['8728932003911564969352217864684', '1928372132132819737213', '8728932001983192837219398127471', 2],
            ['-0.00055999', '9.9999E-10', '-5.6E-4', 8],
            ['15.000000000000311', '3.11e-13', '15', 15],
            ['3110000015', '3.11e9', '15', 0],
        ];
    }

    #[DataProvider('addProvider')]
    public function testAdd(string $expected, string $left, string $right, ?int $scale = 0): void
    {
        self::assertSame($expected, (string)Number::add($left, $right, $scale));
    }

    public function testAddUsingGlobalScale(): void
    {
        Number::setScale(0);
        self::assertSame('2', (string)Number::add('1', '1.05'));
        self::assertSame('2.05', (string)Number::add('1', '1.05', 2));
        Number::setScale(2);
        self::assertSame('2', (string)Number::add('1', '1.05', 0));
        self::assertSame('2.05', (string)Number::add('1', '1.05'));
    }

    public function testSubUsingGlobalScale(): void
    {
        Number::setScale(0);
        self::assertSame('-1', (string)Number::sub('1', '2.5'));
        self::assertSame('-1.5', (string)Number::sub('1', '2.5', 2));
        Number::setScale(2);
        self::assertSame('-1', (string)Number::sub('1', '2.5', 0));
        self::assertSame('-1.5', (string)Number::sub('1', '2.5'));
    }

    public static function subProvider(): array
    {
        return [
            ['-1', '1', '2'],
            ['0', '1', '1'],
            ['5', '10', '5'],
            ['-1.5', '1', '2.5', 2],
            ['-6', '-1', '5', 4],
            ['8728932000054820705086578390258', '8728932001983192837219398127471', '1928372132132819737213', 2],
            ['0.00056', '9.9999E-10', '-5.6E-4', 8],
        ];
    }

    #[DataProvider('subProvider')]
    public function testSub(string $expected, string $left, string $right, ?int $scale = 0): void
    {
        self::assertSame($expected, (string)Number::sub($left, $right, $scale));
    }

    public static function compProvider(): array
    {
        return [
            ['-1', '5', Number::COMPARE_RIGHT_GRATER, 4],
            ['1928372132132819737213', '8728932001983192837219398127471', Number::COMPARE_RIGHT_GRATER, 1],
            ['1.00000000000000000001', '2', Number::COMPARE_RIGHT_GRATER, 1],
            ['97321', '1', Number::COMPARE_LEFT_GRATER, 2],
            ['1', '0', Number::COMPARE_LEFT_GRATER, 0],
            ['1', '1', Number::COMPARE_EQUAL, 0],
            ['0', '1', Number::COMPARE_RIGHT_GRATER, 0],
            ['1', '0', Number::COMPARE_LEFT_GRATER, 0],
            ['1', '1', Number::COMPARE_EQUAL, 0],
            ['0', '1', Number::COMPARE_RIGHT_GRATER, 0],
            ['1', '0.0005', Number::COMPARE_LEFT_GRATER, 4],
            ['1', '0.000000000000000000000000005', Number::COMPARE_LEFT_GRATER, 2],
        ];
    }

    #[DataProvider('compProvider')]
    public function testComp(string $left, string $right, int $expected, int $scale): void
    {
        self::assertSame($expected, Number::compare($left, $right, $scale));
    }

    public static function getScaleProvider(): array
    {
        return [[10], [25], [0]];
    }

    #[DataProvider('getScaleProvider')]
    public function testGetScale(int $expected): void
    {
        Number::setScale($expected);

        self::assertSame($expected, Number::getScale());
    }

    public static function divProvider(): array
    {
        return [
            ['0.5', '1', '2', 2],
            ['-0.2', '-1', '5', 4],
            ['4526580661.75', '8728932001983192837219398127471', '1928372132132819737213', 2],
            ['0.000000000099999', '9.9999E-10', '10', 15],
        ];
    }

    #[DataProvider('divProvider')]
    public function testDiv(string $expected, string $left, string $right, ?int $scale): void
    {
        self::assertSame($expected, (string)Number::div($left, $right, $scale));
    }

    public function testThrowDivByZero(): void
    {
        $this->expectExceptionMessage('Division by zero');
        $this->expectException(InvalidArgumentException::class);
        Number::div('1', '0');
    }

    public function testDivUsingGlobalScale(): void
    {
        Number::setScale(0);
        self::assertSame('0', (string)Number::div('1', '2'));
        self::assertSame('0.5', (string)Number::div('1', '2', 2));
        Number::setScale(2);
        self::assertSame('0', (string)Number::div('1', '2', 0));
        self::assertSame('0.5', (string)Number::div('1', '2'));
    }

    public static function modProvider(): array
    {
        return [
            ['1', '11', '2', 0],
            ['-1', '-1', '5', 0],
            ['1459434331351930289678', '8728932001983192837219398127471', '1928372132132819737213', 0],
            ['0', '9.9999E-10', '1', 0],
            ['0.5', '10.5', '2.5', 2],
            ['0.5', '10.5', '2.5', 2],
            ['0.8', '10', '9.2', 1],
            ['0', '20', '4.0', 1],
            ['0', '10.5', '3.5', 1],
            ['0.3', '10.2', '3.3', 1],
            ['-0.000559999', '9.9999E-10', '-5.6E-4', 9],
        ];
    }

    #[DataProvider('modProvider')]
    public function testMod(string $expected, string $left, string $right, int $scale): void
    {
        self::assertSame($expected, (string)Number::mod($left, $right, $scale));
    }

    public static function mulProvider(): array
    {
        return [
            ['1', '1.5', '1.5', 1],
            ['10', '1.2500', '12.5', 2],
            ['100', '0.29', '29', 0],
            ['100', '0.029', '2.9', 1],
            ['100', '0.0029', '0.29', 2],
            ['1000', '0.29', '290', 0],
            ['1000', '0.029', '29', 0],
            ['1000', '0.0029', '2.9', 1],
            ['2000', '0.0029', '5.8', 1],
            ['1', '2', '2', 2],
            ['-3', '5', '-15', 2],
            ['1234567890', '9876543210', '12193263111263526900', 2],
            ['2.5', '1.5', '3.75', 2],
            ['2.555', '1.555', '3.97', 2],
            ['9.9999E-2', '-5.6E-2', '-0.005599944', 9],
        ];
    }

    #[DataProvider('mulProvider')]
    public function testMul(string $leftOperand, string $rightOperand, string $expected, ?int $scale): void
    {
        self::assertSame($expected, (string)Number::mul($leftOperand, $rightOperand, $scale));
    }

    public static function powProvider(): array
    {
        return [
            ['256', '2', '8', 0],
            ['74.08', '4.2', '3', 2],
            ['-32', '-2', '5', 4],
            ['18446744073709551616', '2', '64', 0],
            ['-108.88', '-2.555', '5', 2],
            ['63998080023999840000.5999988', '19.9999E+2', '6', 9],
            [
                '1229984803535237425357460579824952453848609953896821302286319065669207712270213276022808840210306942692366529569453244416',
                '66',
                '66',
                0,
            ],
            ['1', '0', '0', 0],
            ['0.1', '10', '-1', 1],
            ['1.0837983867343681398392334849264865554733', '5', '0.05', 40],
            ['59.3839', '8', '1.964', 4],
            ['1', '10', '0.0000001', 0],
            ['1', '10', '0.0000001', 2],
            ['36.029', '5.1', '2.2', 3],
            ['0.5492944034820568190269179209773099881437', '1.005', '-120.12345678', 40],
        ];
    }

    #[DataProvider('powProvider')]
    public function testPow(string $expected, string $left, string $right, ?int $scale = 0): void
    {
        self::assertSame($expected, (string)Number::pow($left, $right, $scale));
    }

    public static function logProvider(): array
    {
        return [
            [
                '0.6931471805599453094172321214581765680755001343602552541206800094933936219696947156058633269964186879',
                '2',
            ],
            [
                '2.3025850929940456840179914546843642076011014886287729760333279009675726096773524802359972050895982985',
                '10',
            ],
            ['-INF', '0'],
            ['0', '1'],
            ['NAN', '-1'],
        ];
    }

    #[DataProvider('logProvider')]
    public function testLog(string $expected, string $value): void
    {
        self::assertSame($expected, (string)Number::log($value));
    }

    public static function expProvider(): array
    {
        return [
            [
                '298.8674009670602326720280305552958844792720557285859930698483175291494073935732314440167229005529804941',
                '5.7',
            ],
            [
                '5009718959.9179776145898629945755344777367448017822374820837617183949259570367010764387804812833108859845756998',
                '22.334645654645',
            ],
            [
                '0.3678794411714423215955237701614608674458111310317678345078368016974614957448998033571472743459196437',
                '-1',
            ],
            ['1', '0'],
            [
                '1021450427617659.4094516982518620090788645038742627301331924304676748729927177787220453326541582910229003291582467933',
                '3.456e1',
            ],
        ];
    }

    #[DataProvider('expProvider')]
    public function testExp(string $expected, string $arg): void
    {
        self::assertSame($expected, (string)Number::exp($arg));
    }

    public static function factProvider(): array
    {
        return [
            ['1', 'FOO'],
            ['6', '3'],
            ['24', '4'],
            ['120', '5'],
            ['12696403353658275925965100847566516959580321051449436762275840000000000000', '55'],
            [
                '',
                '2.349e+3',
            ],
        ];
    }

    #[DataProvider('factProvider')]
    public function testFact(string $expected, string $fact): void
    {
        self::assertSame($expected, (string)Number::fact($fact));
    }

    public function testFactThrowErrorOnFloat(): void
    {
        $this->expectExceptionMessage('Number has to be an integer');
        $this->expectException(InvalidArgumentException::class);
        (string)Number::fact('1.1');
    }

    public function testFactThrowErrorOnNegative(): void
    {
        $this->expectExceptionMessage('Number has to be greater than or equal to 0');
        $this->expectException(InvalidArgumentException::class);
        (string)Number::fact('-1');
    }

    public function testPowUsingGlobalScale(): void
    {
        Number::setScale(0);
        self::assertSame('74', (string)Number::pow('4.2', '3'));
        self::assertSame('74.08', (string)Number::pow('4.2', '3', 2));
        Number::setScale(2);
        self::assertSame('74', (string)Number::pow('4.2', '3', 0));
        self::assertSame('74.08', (string)Number::pow('4.2', '3'));
    }

    public static function powModProvider(): array
    {
        return [
            ['4', '5', '2', '7', 0],
            ['4', '5', '2', '7', null],
            ['-4', '-2', '5', '7', 0],
            ['790', '10', '2147483648', '2047', 0],
            ['790', '1E+1', '2E+8', '2047', 0],
            ['4', '5', '2', '7', 2],
            ['3.7', '5', '2', '7.1', 2],
            ['3.7', '5', '2', '7.1', 2],
            ['1', '4', '4', '3', 2],
            ['0.52', '5.1', '2.2', '7.1', 2],
            ['0.52', '5.1', '2.2', '7.1', null],
        ];
    }

    #[DataProvider('powModProvider')]
    public function testPowMod(string $expected, string $left, string $right, string $modulus, ?int $scale): void
    {
        self::assertSame($expected, (string)Number::powMod($left, $right, $modulus, $scale));
    }

    public static function sqrtProvider(): array
    {
        return [
            ['3', '9', 0],
            ['3.07', '9.444', 2],
            ['43913234134.28826', '1928372132132819737213', 5],
            ['0.31', '9.9999E-2', 2],
        ];
    }

    #[DataProvider('sqrtProvider')]
    public function testSqrt(string $expected, string $operand, int $scale): void
    {
        self::assertSame($expected, (string)Number::sqrt($operand, $scale));
    }

    public function testSqrtUsingGlobalScale(): void
    {
        Number::setScale(0);
        self::assertSame('3', (string)Number::sqrt('9.444'));
        self::assertSame('3.07', (string)Number::sqrt('9.444', 2));
        Number::setScale(2);
        self::assertSame('3', (string)Number::sqrt('9.444', 0));
        self::assertSame('3.07', (string)Number::sqrt('9.444'));
    }

    public static function hexdecProvider(): array
    {
        return [
            ['123', '7b'],
            ['1234567890', '499602d2'],
            ['12345678901234567890', 'ab54a98ceb1f0ad2'],
            ['123456789012345678901234567890', '18ee90ff6c373e0ee4e3f0ad2'],
            ['1234567890123456789012345678901234567890', '3a0c92075c0dbf3b8acbc5f96ce3f0ad2'],
            ['1234567890123456789012345678901234567890', '3a0c92075c0dbf3b8acbc5f96ce3f0ad2'],
            [
                '21711016733383976335769713789619838426107890367109022384973563144062439196678',
                '0x300000000d2c12440c4310c20c2428c20c8330cc0ca318ca0cc330cc0c230806',
            ],
        ];
    }

    #[DataProvider('hexdecProvider')]
    public function testHexdec(string $expected, string $operand): void
    {
        self::assertSame($expected, Number::hexdec($operand));
    }

    public static function dechexProvider(): array
    {
        return [
            ['7b', '123'],
            ['ffffffff', '4294967295'],
            ['200000000', '8589934592'],
            ['7fffffffffffffff', '9223372036854775807'],
            ['10000000000000000', '18446744073709551616'],
            ['18ee90ff6c373e0ee4e3f0ad2', '123456789012345678901234567890'],
        ];
    }

    #[DataProvider('dechexProvider')]
    public function testDechex(string $expected, string $operand): void
    {
        self::assertSame($expected, Number::dechex($operand));
    }

    #[DataProvider('bitAddProvider')]
    public function testBitAdd(string $expected, string $left, string $right): void
    {
        self::assertSame($expected, (string)Number::bitAnd($left, $right));
    }

    public static function bitAddProvider(): array
    {
        return [
            [
                '2972225677459078825024027220918272',
                '1000000000865464564564564567867867867800000',
                '5000788676546456456458678760000000',
            ],
            ['610237752474644548', '543543543534543534543534543 ', '4213434324324234324'],
            ['0', '0', '0'],
            ['0', '0', '5'],
            ['1', '1', '5'],
            ['0', '2', '5'],
            ['4', '4', '5'],
            ['4', '-4', '5'],
            ['0', '4', '-5'],
            ['0', '8', '5'],
        ];
    }

    #[DataProvider('bitOrProvider')]
    public function testBitOr(string $expected, string $left, string $right): void
    {
        self::assertSame($expected, (string)Number::bitOr($left, $right));
    }

    public static function bitOrProvider(): array
    {
        return [
            [
                '1000000002894027563651942199302519406881728',
                '1000000000865464564564564567867867867800000',
                '5000788676546456456458678760000000',
            ],
            ['543543547137740106393124319', '543543543534543534543534543 ', '4213434324324234324'],
            ['0', '0', '0'],
            ['5', '0', '5'],
            ['5', '1', '5'],
            ['7', '2', '5'],
            ['5', '4', '5'],
            ['-3', '-4', '5'],
            ['-1', '4', '-5'],
            ['13', '8', '5'],
            ['-853289843298817', '-3213123123123123', '-999696956946954'],
            ['-113449967538441782579763', '-677868678631231237867786867', '-123213213123123123123123'],
        ];
    }

    #[DataProvider('bitXorProvider')]
    public function testBitXor(string $expected, string $left, string $right): void
    {
        self::assertSame($expected, (string)Number::bitXor($left, $right));
    }

    public static function bitXorProvider(): array
    {
        return [
            ['7', '2', '5'],
            ['-8', '3', '-5'],
            ['-6', '-4', '6'],
            ['15', '-8', '-9'],
            ['32111810161015317381218', '21312831290381290382198', '10912839021839123211028'],
            ['-2', '999999999999999999999999999999999', '-999999999999999999999999999999999'],
            ['0', '999999999999999999999999999999999', '999999999999999999999999999999999'],
            ['-2', '-999999999999999999999999999999999', '999999999999999999999999999999999'],
        ];
    }

    public function testThrowErrorOnFloatXorLeftOperator(): void
    {
        $this->expectExceptionMessage('Left operator has to be an integer');
        $this->expectException(InvalidArgumentException::class);
        Number::bitXor('0.001', '1');
    }

    public function testThrowErrorOnNegativeModPowExponent(): void
    {
        $this->expectExceptionMessage('Exponent can\'t be negative');
        $this->expectException(InvalidArgumentException::class);
        Number::powMod('10000.123', '-1', '2');
    }

    public function testThrowErrorOnZeroModPowModulus(): void
    {
        $this->expectExceptionMessage('Modulus can\'t be zero');
        $this->expectException(InvalidArgumentException::class);
        Number::powMod('10000.123', '1', '0');
    }

    public function testThrowErrorOnFloatOrLeftOperator(): void
    {
        $this->expectExceptionMessage('Left operator has to be an integer');
        $this->expectException(InvalidArgumentException::class);
        Number::bitOr('0.001', '1');
    }

    public function testThrowErrorOnFloatAndLeftOperator(): void
    {
        $this->expectException(InvalidArgumentException::class);
        $this->expectExceptionMessage('Left operator has to be an integer');
        Number::bitAnd('0.001', '1');
    }

    public function testThrowErrorOnFloatXorRightOperator(): void
    {
        $this->expectExceptionMessage('Right operator has to be an integer');
        $this->expectException(InvalidArgumentException::class);
        Number::bitXor('1', '0.001');
    }

    public function testThrowErrorOnFloatOrRightOperator(): void
    {
        $this->expectExceptionMessage('Right operator has to be an integer');
        $this->expectException(InvalidArgumentException::class);
        Number::bitOr('1', '0.001');
    }

    public function testThrowErrorOnFloatAndRightOperator(): void
    {
        $this->expectException(InvalidArgumentException::class);
        $this->expectExceptionMessage('Right operator has to be an integer');
        Number::bitAnd('1', '0.001');
    }

    #[DataProvider('convertBinaryProvider')]
    public function testConvertBinary(string $expected, string $base64binary): void
    {
        $decoded = (string)base64_decode($base64binary, true);
        self::assertSame($expected, Number::bin2dec($decoded));
        self::assertSame($decoded, Number::dec2bin($expected));
    }

    public static function convertBinaryProvider(): array
    {
        return [
            ['1000000000865464564564564567867867867800000', 'C3q8Ypr74y+H5r28hvnkbmHA'],
            ['5000788676546456456458678760000000', '9o7TsLbE7mZg8DmCSgA'],
            ['543543543534543534543534543', 'AcGb0ol63k727vnP'],
            ['2', 'Ag=='],
            ['48', 'MA=='],
        ];
    }

    public function testThrowErrorOnIncorrectBaseInBin2Dec(): void
    {
        $this->expectExceptionMessage('Invalid Base: 300');
        $this->expectException(InvalidArgumentException::class);
        Number::bin2dec('', 300);
    }

    public function testThrowErrorOnIncorrectBaseInDec2Bin(): void
    {
        $this->expectException(InvalidArgumentException::class);
        $this->expectExceptionMessage('Invalid Base: 600');
        Number::dec2bin('1', 600);
    }

    public function testCorrectFormatTrailingZeros(): void
    {
        Number::setTrimTrailingZeroes(false);

        self::assertSame('64.0000000000', (string)Number::pow('8', '2', 10));
        self::assertSame('2.0000', (string)Number::sqrt('4', 4));
        self::assertSame('14444.2230000000', (string)Number::add('10000.123', '4444.1', 10));
        self::assertSame('19.524770142330000', (string)Number::mul('9.123767', '2.13999', 15));
        self::assertSame('2.00', (string)Number::div('4.000', '2.0', 2));
        self::assertSame('5556.0230000000', (string)Number::sub('10000.123', '4444.1', 10));
        self::assertSame('1.35', (string)Number::powMod('10000.123', '4444.1', '2'));
        self::assertSame('1111.9230000000', (string)Number::mod('10000.123', '4444.1', 10));
        self::assertSame('-50000000000.0000000000', (string)Number::convertToNumber('-5e+10'));
        self::assertSame('10000.123000', (string)Number::round('10000.123', 6));

        Number::setTrimTrailingZeroes(true);

        self::assertSame('64', (string)Number::pow('8', '2', 10));
        self::assertSame('2', (string)Number::sqrt('4', 4));
        self::assertSame('14444.223', (string)Number::add('10000.123', '4444.1', 10));
        self::assertSame('19.52477014233', (string)Number::mul('9.123767', '2.13999', 15));
        self::assertSame('2', (string)Number::div('4.000', '2.0', 2));
        self::assertSame('5556.023', (string)Number::sub('10000.123', '4444.1', 10));
        self::assertSame('1.35', (string)Number::powMod('10000.123', '4444.1', '2'));
        self::assertSame('1111.923', (string)Number::mod('10000.123', '4444.1', 10));
        self::assertSame('-50000000000', (string)Number::convertToNumber('-5e+10'));
        self::assertSame('10000.123', (string)Number::round('10000.123', 6));
    }
}
